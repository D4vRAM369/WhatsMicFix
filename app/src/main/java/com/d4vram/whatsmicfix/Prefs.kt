...
object Prefs {
    ...
    // Valores predeterminados con 8dB como pediste
    @Volatile var moduleEnabled: Boolean = true
    @Volatile var advancedMode: Boolean = false
    @Volatile var boostDb: Float = 8.0f // 8dB = ~2.5x boost
    @Volatile var boostFactor: Float = 2.51f // 10^(8/20) ≈ 2.51x
    ...
    fun saveFromUi(ctx: Context, enable: Boolean, db: Float, adv: Boolean) {
        // Permitir hasta 12dB (4x) con el nuevo sistema de compresión
    ...
    private fun loadFromXsp(ttlMs: Long) {
        ...
        try {
            ...
            Logx.e("Error cargando XSharedPreferences, usando valores por defecto", t)
            moduleEnabled = true
            boostDb = 8.0f
            boostFactor = dbToLinear(boostDb)
            enablePreboost = true
